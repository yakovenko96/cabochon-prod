...
8. Сдача фиксирует годный вес, выявленный брак, сделанный брак и потери. Потери
   считаются как `weight_before_g - weight_g - defect_weight_g`, где
   `defect_weight_g = detected_defect_weight_g + made_defect_weight_g`.
9. Подтверждение сдачи создает движения прихода, выявленного брака, сделанного
   брака и потерь. Для годного веса и брака создаются результирующие мешки:
   браковые мешки получают `is_defect_lot` и `defect_kind`, остаются на складе
   сдачи и могут быть выбраны технологом для дальнейших операций.
...
Для нефинальных операций склад сдачи берется из последней операции маршрута:
помывки, сортировки и связки помывка+сортировка возвращаются на склад
подготовленного сырья; пресс и пресс+нормализация возвращаются на склад
полуфабрикатов. В сдаче после пресса, включая пресс+нормализацию, заполняются
форма и размер. В сдаче после сортировки заполняются цвет и размер.
...
Менеджеры складов и менеджер готового склада видят заявки также через назначенные
им документы выдачи/сдачи. Менеджер готового склада имеет доступ на изменение
заявок для подтверждения складских действий. Допуски сотрудников к операциям
редактируют только технолог и администратор. Технолог, менеджер склада и
менеджер готового склада могут быть назначены работниками по заявке, если
технолог или администратор заранее добавил им нужные допуски.
...
2026-06-18 выполнены правки сдач, брака, этикеток и прав:

- нефинальные сдачи идут на склад по последней операции, поэтому помывки и
  сортировки возвращаются на склад подготовленного сырья;
- пресс+нормализация показывает и требует поля пресса: форму и размер;
- брак в сдаче разделен на выявленный и сделанный, оба типа создают отдельные
  браковые мешки на складе сдачи и движения с `defect_kind`;
- этикетка мешка больше не печатает слово "Мешок" и работника, а атрибуты
  печатает только если они заполнены на мешке;
- менеджер готового склада получил доступ к заявкам, складские менеджеры видят
  заявки через назначенные выдачи/сдачи; допуски к операциям редактируют только
  технолог и администратор, но технолог и любые менеджеры складов могут быть
  назначены работниками при наличии допуска;
- проверка выполнена: `ruff passed`, `compileall passed`, `xml/csv parse passed`,
  `module update passed`, `odoo restarted`.
